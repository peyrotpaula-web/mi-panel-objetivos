...
maestro_asesores = {
    ...
    "968 RODRIGO JULIAN RIOS": "GRANVILLE MADRYN", "974 CIELO QUIROGA": "FORTECAR SAN NICOLAS",
    ...
    "1119 ROMAN GAVINO": "FORTECAR NUEVE DE JULIO", "658 BRUNO GONZALEZ": "GERENCIA",
    ...
    "1121 MARTIN EZEQUIEL POTREBICA": "FORTECAR NUEVE DE JULIO", "564 GOMEZ URIEL": "GERENCIA",
    "1128 JONATHAN LUIS PEREZ": "FORTECAR NUEVE DE JULIO", "1105 ANGEL AUGUSTO FRANCO": "GRANVILLE TRELEW",
    "1049 BRUNO VIGNALE": "OPENCARS JUNIN", "1129 MARIANO MENA": "FORTECAR TRENQUE LAUQUEN",
    "406 TOMÁS DI NUCCI": "FORTECAR JUNIN", "1119 ROMAN GAVINO": "FORTECAR NUEVE DE JULIO",
    "1131 PABLO DEMARIA": "GRANVILLE JUNIN", "1132 VALERIA NATALIA DINARDO": "GRANVILLE PERGAMINO",
    "1132 VALERIA NATALIA DINARDO": "GRANVILLE PERGAMINO", "TRINIDAD SHANAHAN": "SUCURSAL VIRTUAL", 
    "872 AGUSTIN POCHELU": "GRANVILLE TRELEW", "JULIETA CORREA": "SUCURSAL VIRTUAL", "CAMILA MARTINEZ": "SUCURSAL VIRTUAL", 
    "410 NICOLÁS FERREYRA": "FORTECAR PERGAMINO", "1113 JULIO DE LA TORRE": "GRANVILLE COMODORO", 
    "428 LOURDES BARRA SUEIRO": "GERENCIA", "837 VICTOR HUGO HERRERA": "FORTECAR TRENQUE LAUQUEN", "450 MARCOS GOMEZ": "GERENCIA",
    "1140 GONZALO MARTIN MAURO": "FORTECAR CHIVILCOY", "1144 FRANCO FIGUEROA": "FORTECAR OLAVARRIA", 
    "1143 MATIAS LEZAETA": "HUB PDA", "1141 MORENA VIDELA": "HUB PDA", "1142 MAURICIO LEIVA": "HUB PDA", 
    "LOURDES RAGGIO": "SUCURSAL VIRTUAL", "MORENA VIDELA": "SUCURSAL VIRTUAL", "MARTIN CASANOVAS": "SUCURSAL VIRTUAL",
    "1148 SAMUEL RAMBOSIO": "FORTECAR NUEVE DE JULIO", "1146 JUAN ANDRES MORALES": "GRANVILLE CITROEN SAN NICOLAS",
    "CRISTIAN LOSAURO": "SUCURSAL VIRTUAL", "1149 ALDANA GUERRA": "GRANVILLE CITROEN SAN NICOLAS",
    "1151 MAURO ALBERTO LOPEZ": "PAMPAWAGEN GENERAL PICO", "1153 SEBSTIAN ALEXIS OLIVETTI": "PAMPAWAGEN GENERAL PICO","1152 SEBASTIAN ALEXIS OLIVETTI": "PAMPAWAGEN GENERAL PICO",
    "1155 TOBIAS RODRIGUEZ": "FORTECAR SAN NICOLAS", "CANDELA BOFFI": "SUCURSAL VIRTUAL", "MAURICIO LEIVA": "SUCURSAL VIRTUAL",
    "1156 GERMAN CALVO": "HUB PDA", "MATIAS LEZAETA": "SUCURSAL VIRTUAL", "1157 JUAN IGNACIO DIEZ": "HUB PDA", 
    "MICHELLE ANTONELLA PARDO": "SUCURSAL VIRTUAL", "1159 CARLA VALLEJO": "HUB PDA", "1101 RODRIGO BACCHIARRI": "GRANVILLE TRELEW",
    "542 GARCIA NICOLAS": "PAMPAWAGEN SANTA ROSA", "GERARDO GONZALEZ": "SUCURSAL VIRTUAL", "1165 CLAUDIO ANDRES SULETA": "FORTECAR PERGAMINO",
    "1162 TAMARA GENOVEVA LOPEZ": "GRANVILLE CITROEN SAN NICOLAS", "1166 GERARDO ANDRES GONZALEZ": "HUB PDA",
    "1168 LAUTARO CANDAL BARCIA": "FORTECAR OLAVARRIA", "1135 GASTON SENOSEAIN": "PAMPAWAGEN SANTA ROSA",
...
if 'df_panel_datos' not in st.session_state:
    st.session_state['df_panel_datos'] = None
...
# =========================================================
    ...
    if f_meta:
        try:
            ...
            st.session_state['df_panel_datos'] = df_m.copy()
            def resaltar_totales(row):
                if "TOTAL" in str(row[cols[0]]).upper(): return ['font-weight: bold; background-color: #f0f2f6'] * len(row)
                return [''] * len(row)
            ...
            estilo_df = df_final.style.apply(resaltar_totales, axis=1).map(semaforo_fuente, subset=[col_pct_n1, col_pct_n2]).format({cols[1]: "{:,.0f}", cols[2]: "{:,.0f}", cols[3]: "{:,.0f}", col_pct_n1: "{:.1%}", col_pct_n2: "{:.1%}", "Faltante N1": "{:,.0f}", "Faltante N2": "{:,.0f}"})
            st.dataframe(estilo_df, use_container_width=True, hide_index=True, height=(len(df_final) + 1) * 36)
        except Exception as e: st.error(f"Error: {e}")

# =========================================================
# OPCIÓN 3: PANEL DE OBJETIVOS SUCURSALES (AJUSTADO)
# =========================================================
elif pagina == "Panel de Objetivos Sucursales":
    st.title("📊 Panel de Control de Objetivos Sucursales")
    if st.session_state['df_panel_datos'] is None:
        st.warning("⚠️ Sin datos. Por favor, procesa primero el Panel 'Cumplimiento de Objetivos 🎯'.")
    else:
        ...
        st.subheader("Matriz de Seguimiento (Ordenada por %)")
        m1, m2 = st.columns(2)
        with m1:
